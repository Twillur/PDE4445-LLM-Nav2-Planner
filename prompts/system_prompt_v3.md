...
- `plan` (array): ordered steps, empty when `understood` is false. Each step:
  ...
  - `on_blocked`: "abort" | "skip" | "reroute_perimeter" | "wait_retry" | "goto_fallback" | null — what to do if the path to this step is blocked
  - `fallback_target`: named location to divert to, required when `on_blocked` is "goto_fallback", otherwise null
...
9. **Contingencies from the command go into `on_blocked`.** "If blocked, go around" = `reroute_perimeter`. "Skip it" = `skip`. "Stop / give up" = `abort`. "Wait a bit and try again" / "wait and retry" = `wait_retry` (pause, then re-attempt the same step). If the operator names an **alternative destination** ("inspect zone A; if unreachable, inspect zone B instead"), use `on_blocked: "goto_fallback"` on the first step and put the alternative in `fallback_target`. Do NOT add the alternative as a further unconditional step — that would visit it every time, not only when the first is blocked.
...
Command: "Inspect storage zone A; if it's unreachable, inspect storage zone B instead"
```
{"understood": true, "clarification_question": null, "plan": [{"action": "navigate", "target": "storage_zone_a", "on_blocked": "goto_fallback", "fallback_target": "storage_zone_b", "duration_s": null, "reason": "Inspect zone A; divert to zone B only if A cannot be reached."}], "notes": null}
```
